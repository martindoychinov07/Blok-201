...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<AuthDto.ErrorResponse> handleDisabled(AuthenticationException ex) {
        log.warn("Authentication unavailable: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
    ...
    public ResponseEntity<AuthDto.ErrorResponse> handleBadPayload(HttpMessageNotReadableException ex) {
        log.warn("Bad request payload: {}", ex.getMessage());
        return ResponseEntity.badRequest().body(new AuthDto.ErrorResponse(
                "Invalid request payload. Check role value, patient UUID format, and JSON fields."
        ));
    }
